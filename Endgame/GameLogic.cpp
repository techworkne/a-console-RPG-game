...
bool GameLogic::hasForbiddenExtension(const std::string& filename) const{
    if (filename.size() > 1 && filename[0] == '.') {
        std::string rest = filename.substr(1);
        return (rest == "cpp" || rest == "hpp" || rest == "h" || rest == "c");
    }
...
bool GameLogic::SaveToFile(const std::string& filename) const {
    if (hasForbiddenExtension(filename)) {
        std::cout << "Error: Cannot save to this ext!\n";
        return false;
    }
