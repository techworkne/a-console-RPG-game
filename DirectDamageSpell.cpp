...
DirectDamageSpell::DirectDamageSpell(int damage, int mana_cost) : damage_(damage), mana_cost_(mana_cost), enhanced_(false) {}
...
bool DirectDamageSpell::cast(GameField& field, Entity* caster, size_t target_x, size_t target_y) {
    ...
    target->TakeDamage(enhanced_ ? damage_ * 2 : damage_);
    std::cout << "Direct damage spell! Done " << (enhanced_ ? damage_ * 2 : damage_) << " damage. " << target->getEntityTypeName(target->GetType()) << " HP: " << target->GetHealth() << "\n";

    if (target->GetType() != EntityType::kPlayer && !target->IsAlive()) {
        Player* player = dynamic_cast<Player*>(caster);
        RewardSystem::GiveDestructionReward(target, player);
